...
- 🧪 **模板在线调试**：在模板编辑页直接输入测试数据，实时预览渲染结果，调试模板更高效
...
| 🎬 **Emby / Jellyfin** | 播放/入库事件智能识别，自动生成图片 URL，独立模板 |
...
**🧪 在线调试**：在模板编辑页点击「预览」按钮，输入测试数据即可实时查看渲染结果，无需实际发送通知。
...
- 🧪 **模板在线调试**：模板编辑页内置预览功能，输入测试数据实时查看渲染结果
